#include "Tweener.h"
/*****************
	2015 Version fixed by Juan Belón Pérez - from xixgames.com
...
namespace tweeners {
	...
	void Tweener::step(long currentMillis) {
		...
		tweensIT = tweens.begin();
		if(tweensIT==tweens.end()) return;
		
		//for (;  tweensIT != tweens.end(); ++tweensIT ) 
		for(int i=0; i<total_tweens; i++)
		{
			if (!(*tweensIT).started) 
			{
